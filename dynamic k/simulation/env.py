...
@dataclass(frozen=True)
class AcceptedPath:
    """Accepted speculative prefix statistics on the verifier-followed branch."""
...
class TreeSpeculativeDecodingEnv:
    ...
    def sample_accepted_tokens(self, k: int, state: dict[str, Any]) -> AcceptedPath:
        """Sample accepted speculative depth on the verifier-followed branch."""
    ...
    def step(self, state: dict[str, Any], k: int) -> tuple[dict[str, Any], float, bool, dict[str, Any]]:
        ...
        emitted_tokens = 1 + accepted.accepted_tokens
